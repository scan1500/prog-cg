...
function Circle(canvas_)
{
	...
	void main() 
	{
		...
		v_localCoordinates = a_position.xy * u_scale.xy;
	}
	`;
	...
	void main() 
	{
		float l = length(v_localCoordinates.xy);
		
		if(l < 1.0)
		{
			if(l < 1.0 - uBorderRadius)
			{
	...
	function initScene()
	{
		...
		mUScale = gl.getUniformLocation(mProgram, "u_scale");
		mUBorderRadius = gl.getUniformLocation(mProgram, "uBorderRadius");
	...
	function drawScene(timer)
	{
		...
		var borderSize = document.getElementById("borderSizeSlider").value;
		

		if(mCanvas.width > mCanvas.height)
		{
			gl.uniform2fv(mUScale, [mCanvas.width / mCanvas.height, 1.0]);
			gl.uniform1fv(mUBorderRadius, [2.0 * borderSize / (mCanvas.height)]);
		}
		else
		{
			gl.uniform2fv(mUScale, [1.0,  mCanvas.height / mCanvas.width]);
			gl.uniform1fv(mUBorderRadius, [2.0 * borderSize / (mCanvas.width)]);
		}
